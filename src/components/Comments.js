...
function Comments(props) {
    ...
    const addComments= async()=>{
        ...
        try{
           auth.currentUser && await addDoc(commentsRef,{
            ...
            auth.currentUser && toast.success("comments added successfully")
        }catch(err){
            console.log(err)
        }
    }
  ...
  return (
    <div className='grid grid-rows-2'> 
       <div className='p-5'>
            <label for="Add Comments" class="block mb-2 text-sm font-medium text-gray-900 dark:text-black">Add Comments</label>
            <div className='flex'>
            ...
            <button onClick={addComments} className=" ml-2 bg-gray-50 hover:bg-slate-300 text-gray-900 text-sm border border-gray-300 py-2 px-4 rounded">
            Add
</button>
            </div>
